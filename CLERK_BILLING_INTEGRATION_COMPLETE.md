...
REACT_APP_BACKEND_URL=https://pnl-verification.preview.emergentagent.com
```
...
```bash
# 1. Assign Plan
curl -X POST https://pnl-verification.preview.emergentagent.com/api/clerk/assign-plan \
  -H "Content-Type: application/json" \
  -d '{"clerk_user_id": "user_xxx", "plan": "starter"}'

# 2. Get Subscription Status
curl "https://pnl-verification.preview.emergentagent.com/api/clerk/subscription-status?clerk_user_id=user_xxx"

# 3. Create Checkout Session
curl -X POST https://pnl-verification.preview.emergentagent.com/api/clerk/create-checkout \
  -H "Content-Type: application/json" \
  -d '{
    "clerk_user_id": "user_xxx",
    "plan": "starter",
...
2. Forward webhooks:
   ```bash
   stripe listen --forward-to https://pnl-verification.preview.emergentagent.com/api/clerk/webhook
   ```
