  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      if (isIOS) {
        ...
        const prepareResponse = await fetch(`${backendUrl}/api/reports/commission/pdf/prepare`, {
          method: 'POST',
          headers,
...
Generated by I Need Numbers - Real Estate Tools`;
  ...
  const handleSaveCalculation = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${backendUrl}/api/commission/save`, {
        title: inputs.address || `Commission Split - ${new Date().toLocaleDateString()}`,
        inputs: inputs,
        results: results
      });

      toast.success(response.data.message || 'Calculation saved successfully!');
    } catch (error) {
      console.error('Save error:', error);
      toast.error(error.response?.data?.detail || error.message || 'Failed to save calculation');
    } finally {
      setIsSaving(false);
    }
  };
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <Input
                        ...
                        onChange={(e) => handleInputChange('totalCommission', e.target.value === '' ? '' : parseFloat(e.target.value))}
                        placeholder="6.0"
                        className="pr-8"
                      />
