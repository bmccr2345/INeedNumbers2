          ...
          <div className="lg:col-span-2 space-y-6">
            <Card>
              ...
              <CardContent className="space-y-6">
                ...
                {/* AI Coach Reminders based on loan type - full width */}
                <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
                  <p className="text-sm text-blue-700">
                    <span className="font-semibold">AI Coach Reminders: </span>
                    {inputs.loanType === 'conventional' && (
                      <>Not backed by the government. They follow Fannie Mae/Freddie Mac guidelines. Best for people with 620+ credit scores and 5% or more down payment. If you put down less than 20%, you pay PMI.</>
                    )}
                    {inputs.loanType === 'va' && (
                      <>Guaranteed by the U.S. Department of Veterans Affairs. It offers 0% down payment, no private mortgage insurance (PMI), and competitive interest rates, making it highly affordable for eligible veterans.</>
                    )}
                    {inputs.loanType === 'fha' && (
                      <>Insured by the Federal Housing Administration, designed for low-to-moderate-income borrowers. Requires 3.5% down (if 580+ credit) and allows lower credit scores (down to 500), but requires upfront and monthly mortgage insurance.</>
                    )}
                    {inputs.loanType === 'usda' && (
                      <>Backed by the U.S. Department of Agriculture for specific rural and suburban areas. Requires no down payment but has strict income limits (cannot exceed 115% of the median household income in that area).</>
                    )}
                    {inputs.loanType === 'jumbo' && (
                      <>These are for financing homes that exceed the loan limits set by the FHFA (over $832,750 in most counties in 2026). These are riskier for lenders, so they require higher credit scores and significant down payments.</>
                    )}
                  </p>
                </div>
              </CardContent>
