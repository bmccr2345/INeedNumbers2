    ...
    def test_branding_upload_validation(self):
        ...
        try:
            ...
            if validation_passed >= validation_total - 1:  # Allow one failure
                print(f"   ✅ Validation working correctly ({validation_passed}/{validation_total})")
                return True, validation_results
            else:
                print(f"   ❌ Validation issues detected ({validation_passed}/{validation_total})")
                return False, validation_results
    ...
    def test_function_imports(self):
        ...
        try:
            ...
            if pdf_response.status_code == 500:
                try:
                    error_response = pdf_response.json()
                    print(f"   ❌ PDF generation error: {error_response}")
                ...
                print(f"   ✅ PDF generation working (status: {pdf_response.status_code})")
