...
def test_security_headers():
    ...
    try:
        response = requests.get("https://pnl-verification.preview.emergentagent.com/api/health", timeout=10)
...
def test_core_apis(auth_token):
    ...
    for name, method, endpoint, needs_auth in endpoints:
        try:
            url = f"https://pnl-verification.preview.emergentagent.com{endpoint}"
...
def test_plan_access_control(auth_token):
    ...
    try:
        response = requests.get(
            "https://pnl-verification.preview.emergentagent.com/api/pnl/deals?month=2025-01",
            headers=headers,
            timeout=10
        )
...
def test_cors_allowlist():
    ...
    try:
        response = requests.options(
            "https://pnl-verification.preview.emergentagent.com/api/health",
            headers={
...
def test_webhook_endpoint():
    ...
    try:
        response = requests.post(
            "https://pnl-verification.preview.emergentagent.com/api/stripe/webhook",
            json=webhook_data,
            timeout=10
        )
